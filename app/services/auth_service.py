...
from exceptions.auth import InvalidCredentials, UserNotFound
...
def verify_otp_and_signup(db, request: VerifyOTPRequest):
    ...
    access_data = {"mobile": user.mobile_number, "role": user.role}
...
def login_user(db, login: UserLogin):
    user = user_crud.get_user_by_mobile(db, login.mobile_number)
    login_hashed = get_password_hash(login.password)
    if not user or not verify_password(login.password, user.hashed_password):
        access_history.create_access_history(db, login.mobile_number, login_hashed, AccessStatus.FAILED.value)
        # raise Exception("Invalid credentials")
        raise UserNotFound()
    access_history.create_access_history(db, login.mobile_number, login_hashed, AccessStatus.SUCCESSFUL.value)
    access_data = {"mobile_number": str(user.mobile_number), "role": f"{user.role}"}
    # return create_access_token({"sub": str(user.id), "role": user.role})
    return {
        "access_token": create_access_token(access_data),
        "refresh_token": create_refresh_token(access_data)
...
def logout_user(db, login: UserLogout):
    try:
        ...
        if not user:
            return False
        access_history.create_access_history(db, login.mobile_number, None, AccessStatus.LOGOUT.value)
        return True
    except Exception as e:
        raise Exception(str(e))
